...
enum TokenKind { UNKNOWN, KEYWORD, INTEGER, VARIABLE, STRING, BOOLEAN};
...
private:
	...
	list<Token> dowhilesave; //keeps the tokens for while
	...
	static void fillTK(Sally * Sptr); //simple function
	...
	static void doGreaterEqu(Sally *Sptr);
	static void doLesser(Sally *Sptr);
	static void doLesserEqu(Sally *Sptr);
	...
	static void doExclaim(Sally *Sptr);
